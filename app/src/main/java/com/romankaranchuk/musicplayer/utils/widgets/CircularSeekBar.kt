...
class CircularSeekBar @JvmOverloads constructor(
        ...
        set(max) {
            if (max > 0) {
                if (max <= progressActual) {
                    progressActual = min // If the new max is less than current progress, set progress to zero
    ...
    private fun initAttributes(attrArray: TypedArray) {
        ...
        min = attrArray.getInt(R.styleable.cs_CircularSeekBar_cs_min, DEFAULT_MIN).toFloat()
        progressActual =
