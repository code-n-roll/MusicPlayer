...
public class MusicStoreImpl implements MusicStore {
    ...
    private static MusicStoreImpl INSTANCE;


    private SQLiteOpenHelperImpl mDbHelper;

    private MusicStoreImpl(@NonNull Context context){
        mDbHelper = new SQLiteOpenHelperImpl(context);
    }

    public static MusicStoreImpl getInstance(@NonNull Context context){
        if (INSTANCE == null){
            INSTANCE = new MusicStoreImpl(context);
        }
        return INSTANCE;
    }
